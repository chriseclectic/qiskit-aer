...
#ifndef _aer_tensor_indexing_hpp_
#define _aer_tensor_indexing_hpp_
...
//============================================================================
// Qubit vector indexing Functions
//============================================================================
...
template<typename R>
uint_t index0(const uint_t k, const R &qubits_sorted);

// Return a std::vector of 2^N in ints
...
dynamic_reg_t indexes(const dynamic_reg_t &qubits, const dynamic_reg_t &qubits_sorted, const uint_t k);

// As above but returns a fixed sized array of of 2^N in ints
template<size_t N>
static_reg_t<1ULL << N> indexes(const static_reg_t<N> &qubit,
                                const static_reg_t<N> &qubits_sorted,
                                const uint_t k);

template<typename R>
dynamic_reg_t controlled_indexes(const dynamic_reg_t& target_qubits,
                                 const R& control_qubits,
                                 const R& qubits_sorted,
                                 const uint_t k,
                                 const uint_t control_val);

template<size_t N, typename R>
static_reg_t<1ULL << N> controlled_indexes(const static_reg_t<N>& target_qubits,
                                           const R& control_qubits,
                                           const R& qubits_sorted,
                                           const uint_t k,
                                           const uint_t control_val);
...
uint_t index0(const reg_t &qubits_sorted, const uint_t k) {
...
template <size_t N>
static_reg_t<1ULL << N> indexes(const static_reg_t<N> &qubits,
                                const static_reg_t<N> &qubits_sorted,
                                const uint_t k) {
  static_reg_t<BITS[N]> ret;
...
dynamic_reg_t indexes(const dynamic_reg_t &qubits,
                      const dynamic_reg_t &qubits_sorted,
                      const uint_t k) {
  const auto N = qubits_sorted.size();
  dynamic_reg_t ret(BITS[N]);
  // Get index0
  ret[0] = index0(qubits_sorted, k);
  for (size_t i = 0; i < N; i++) {
    const auto n = BITS[i];
...
dynamic_reg_t controlled_indexes(const dynamic_reg_t& target_qubits,
  ...
  for (size_t i = 0; i < N; i++) {
    const auto n = BITS[i];
    const auto bit = BITS[target_qubits[i]];
    for(size_t j=0; j<<n; j++) {
        ret[n + j] = ret[j] | bit;
    }
  }
