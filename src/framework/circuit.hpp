...
public:
  ...
  json_t header;                // Circuit header metdata TODO: remove from class
  uint_t shots = 1;             // Number of shots for execution TODO: move to qobj/controller
  uint_t seed;                  // RNG seed for execution  TODO: move to qobj/controller
...
void from_json(const json_t &js, Circuit &circ) {
  ...
  if (JSON::check_key("header", js)) {
    ...
    JSON::get_value(circ.name, "name", circ.header);
  }
