...
const Operations::OpSet StateOpSet(
    ...
    {"statevector", "memory", "register", "probabilities",
     "probabilities_conditional", "probabilites_single_shot", "density_matrix",
     "density_matrix_conditional", "expectation_value_pauli",
     "expectation_value_pauli_conditional",
     "expectation_value_pauli_single_shot"
     "expectation_value_matrix",
     "expectation_value_matrix_conditional",
...
void State<statevec_t>::apply_snapshot(const Operations::Op &op,
  ...
  case Snapshots::densmat: {
    snapshot_density_matrix(op, data, DataType::Average);
  } break;
  case Snapshots::densmat_cond: {
    snapshot_density_matrix(op, data, DataType::Conditional);
  } break;
  ...
  case Snapshots::expval_matrix: {
    snapshot_matrix_expval(op, data, DataType::Average);
  } break;
  case Snapshots::expval_matrix_cond: {
    snapshot_matrix_expval(op, data, DataType::Conditional);
  } break;
  case Snapshots::expval_matrix_shot: {
    snapshot_matrix_expval(op, data, DataType::Pershot);
  } break;
  default:
...
void State<statevec_t>::snapshot_pauli_expval(const Operations::Op &op,
  ...
  BaseState::add_additional_data(op.string_params[0], type, std::move(expval),
                                 data);
}
...
template <>
cmatrix_t
State<QV::QubitVectorThrust<float>>::density_matrix(const reg_t &qubits) {
  return vec2density(qubits, BaseState::qreg_.vector());
}

template <>
cmatrix_t
State<QV::QubitVectorThrust<double>>::density_matrix(const reg_t &qubits) {
  return vec2density(qubits, BaseState::qreg_.vector());
}
#endif
