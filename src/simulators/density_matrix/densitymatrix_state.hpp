...
const Operations::OpSet StateOpSet(
    ...
    // Gates
    {"U", "CX", "u1", "u2", "u3", "cx", "cz", "swap", "id", "x", "y", "z", "h",
     "s", "sdg", "t", "tdg", "ccx"},
    // Snapshots
    {"density_matrix", "memory", "register", "probabilities",
     "probabilities_single_shot", "probabilities_conditional",
     "expectation_value_pauli", "expectation_value_single_shot",
...
void State<densmat_t>::snapshot_density_matrix(const Operations::Op &op,
  ...
  BaseState::add_additional_data(op.string_params[0], DataType::Average,
...
void State<densmat_t>::apply_gate_u3(uint_t qubit, double theta, double phi,
                                     double lambda) {
  BaseState::qreg_.apply_unitary_matrix(reg_t({qubit}),
                                        Utils::VMatrix::u3(theta, phi, lambda));
}
